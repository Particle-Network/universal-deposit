...
async function getJwtForUser(userId: string): Promise<string> {
  const res = await fetch(`/api/particle-jwt?uid=${encodeURIComponent(userId)}`, {
    credentials: 'include',
  });
  if (!res.ok) throw new Error('Failed to fetch JWT from backend');
  const data = await res.json();
  if (!data.jwt) throw new Error('Backend did not return jwt field');
...
function App() {
  ...
  const { connect: jwtConnect, userInfo, connected: particleConnected } = useConnect();
  ...
  const selectedChain = useMemo(
    () => DEPOSIT_CHAIN_OPTIONS.find((c) => c.chainId === selectedChainId) ?? DEPOSIT_CHAIN_OPTIONS[0],
    [selectedChainId]
  );
  ...
  const depositAddress = selectedChain.addressType === 'solana' ? uaSolAddr : uaEvmAddr;

  // --- Login Logic ---
  useEffect(() => {
    const run = async () => {
      if (!isConnected || !connectkitEoa) return;
      if (particleConnected || userInfo) return;
    ...
    void run();
  }, [isConnected, connectkitEoa, particleConnected, userInfo, jwtConnect]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const evm = options?.evmSmartAccount ?? options?.smartAccountAddress ?? null;
        const sol = options?.solanaSmartAccount ?? options?.solanaSmartAccountAddress ?? null;
        setUaEvmAddr(evm);
        setUaSolAddr(sol);
      } catch (e) {
  ...
  // --- Modal Control ---
  const canOpenDeposit = Boolean(isConnected && connectkitEoa && ua && uaEvmAddr && uaSolAddr);
    ...
    try {
      ...
      for (const a of assets) {
        ...
        for (const ca of chainAgg) {
          ...
          if (chainId > 0 && raw > 0n && valueUsd > 0.50) {
             const key = `${tokenType}:${chainId}`;
             if (!processingKeysRef.current.has(key)) {
                console.log(`[CHECK] Found existing ${tokenType} on chain ${chainId} ($${valueUsd})`);
                foundDeltas.push({
                  key,
                  tokenType,
                  chainId,
      ...
      for (const d of foundDeltas) {
        ...
        setProcessing((p) => [{ id, detectedAt: Date.now(), delta: d, status: 'detected' }, ...p]);
  ...
  function extractSnapshot(primaryAssets: PrimaryAssetsResponse): Map<string, bigint> {
    const snap = new Map<string, bigint>();
    const assets = primaryAssets?.assets ?? primaryAssets?.data?.assets ?? primaryAssets?.result?.assets ?? [];
    for (const a of assets) {
      const tokenType = normalizeTokenType(a?.tokenType || a?.token?.type);
      if (!tokenType) continue;
      const chainAgg = a?.chainAggregation ?? a?.chains ?? [];
  ...
  function diffSnapshot(prev: Map<string, bigint>, next: Map<string, bigint>): DetectedDelta[] {
    ...
    for (const [key, nextAmt] of next.entries()) {
      ...
      if (nextAmt > prevAmt) {
        ...
        deltas.push({
          ...
          displaySymbol: tokenTypeToSymbol(normalizeTokenType(tokenTypeRaw) as PrimaryAssetType),
        });
      }
    }
    return deltas;
  ...
  function startPolling() {
    stopPolling();
    pollingRef.current = window.setInterval(() => { void pollOnce(); }, 8000);
    void pollOnce();
  }
  ...
  async function sweepToArbitrum(tokenType: PrimaryAssetType, rawAmount: bigint, sourceChainId: number): Promise<string> {
    if (!ua || !authCoreProvider || !connectkitEoa) throw new Error('Deps not ready');
    ...
    console.log(`[SWEEP] Detected ${tokenType} on ${sourceChainId}. Initiating sweep protocol...`);

    try {
      // Helper to build TX
      const buildTx = async (amountHumanReadable: string, destChainId: number, destTokenAddr?: string) => {
        ...
        if (tokenType === 'eth') {
           ...
           return await (ua as any).createTransferTransaction({
              token: { chainId: destChainId, address: '0x0000000000000000000000000000000000000000' },
      ...
      if (tokenType === 'usdc' && destConfig?.usdc) targets.push({ chainId: TARGET_CHAIN, addr: destConfig.usdc, label: 'Arb Native USDC' });
      else if (tokenType === 'usdt' && destConfig?.usdt) targets.push({ chainId: TARGET_CHAIN, addr: destConfig.usdt, label: 'Arb USDT' });
      else if (tokenType === 'eth') targets.push({ chainId: TARGET_CHAIN, addr: undefined, label: 'Arb ETH' });

      // 2. Target: Bridged Asset (USDC.e) on Arbitrum
      if (tokenType === 'usdc' && destConfig?.usdc_e) {
        targets.push({ chainId: TARGET_CHAIN, addr: destConfig.usdc_e, label: 'Arb Bridged USDC.e' });
      }

      // 3. Fallback: Source Chain
      if (tokenType === 'eth') targets.push({ chainId: sourceChainId, addr: undefined, label: 'Source Chain Fallback' });
      else if (sourceConfig?.[tokenType]) targets.push({ chainId: sourceChainId, addr: sourceConfig[tokenType], label: 'Source Chain Fallback' });
      ...
      for (const target of targets) {
        for (const pct of percentages) {
          try {
            ...
            console.log(`[SWEEP] Attempt: ${target.label} (${safePct}%) -> ${amountArg}`);
            ...
            return `Swept to ${target.chainId === TARGET_CHAIN ? 'Arbitrum' : 'User Wallet'}`;
          } catch (e) {
  ...
  async function handleDelta(itemId: string, d: DetectedDelta) {
    if (!connectkitEoa) {
      setProcessing((p) => p.map((x) => (x.id === itemId ? { ...x, status: 'error', message: 'Wallet not ready' } : x)));
      return;
    }

    setProcessing((p) => p.map((x) => (x.id === itemId ? { ...x, status: 'processing' } : x)));
    setStatusLine('Processing deposit…');

    try {
      if (['eth', 'usdc', 'usdt'].includes(d.tokenType)) {
        const resultMsg = await sweepToArbitrum(d.tokenType, d.amount, d.chainId);

        setProcessing((p) =>
          p.map((x) => (x.id === itemId ? { ...x, status: 'done', message: resultMsg } : x))
        );
      } else if (d.tokenType === 'sol') {
         setProcessing((p) => p.map((x) => (x.id === itemId ? { ...x, status: 'rejected', message: 'Manual SOL withdrawal needed' } : x)));
      } else {
        setProcessing((p) =>
          p.map((x) => (x.id === itemId ? { ...x, status: 'rejected', message: 'Asset not handled' } : x))
        );
      }
    } catch (e: any) {
      setProcessing((p) =>
        p.map((x) => (x.id === itemId ? { ...x, status: 'error', message: 'Transaction Failed' } : x))
      );
    } finally {
      setStatusLine('');
    }
  ...
  return (
    <div className="App">
        ...
        {ready && (
          !isConnected ? (
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={logout} style={ghostBtnSmall}>Disconnect</button>
            </div>
          )
        )}
      </div>
      ...
      <button
        style={{ ...primaryBtn, marginTop: 24, padding: '14px 28px', fontSize: 15 }}
        onClick={openDeposit}
        ...
        <div style={backdrop}>
          <div style={sheet}>
            <div style={headerRow}>
              ...
              <button style={closeIconBtn} onClick={closeDeposit} aria-label="Close">
                <XIcon />
              </button>
            </div>
            ...
            <div style={{ position: 'relative', zIndex: 50 }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 15, fontWeight: 500 }}>{selectedChain.label}</span>
                </div>
              ...
              {isDropdownOpen && (
                <div className="custom-scrollbar" style={customSelectMenu}>
                  {DEPOSIT_CHAIN_OPTIONS.map((c) => {
                    const supportedTokens = PRIMARY_ASSETS_BY_CHAIN[c.chainId] || [];
                    return (
                      <div 
                        key={c.chainId} 
            ...
            {(statusLine || processing.some((p) => p.status === 'processing')) && (
              <div style={statusPill}>
                <SpinnerMini />
                <span style={{ fontSize: 13, fontWeight: 500 }}>{statusLine || 'Processing deposit…'}</span>
              </div>
            )}
            ...
            <div style={{ marginTop: 24 }}>
              ...
              <div style={hintRow}>
                <span style={{ ...hintDot, background: selectedChain.addressType === 'solana' ? '#14F195' : '#627EEA' }} />
                <span style={hintText}>
                  Send <b>{selectedChain.addressType === 'solana' ? 'Solana' : 'EVM'}</b> assets to this address.
                </span>
              </div>
            </div>
            ...
            <div>
              <div style={{ ...fieldLabel, marginBottom: 12 }}>Recent Activity</div>
              {processing.length === 0 ? (
                ...
                <div style={activityList}>
                  {processing.slice(0, 3).map((p) => {
                    const chainMeta = CHAIN_META[p.delta.chainId] ?? { name: `Chain ${p.delta.chainId}`, icon: '' };
                    return (
                      <div key={p.id} style={activityItem}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <div style={activityIconFrame}>
                            <TokenAvatar symbol={p.delta.displaySymbol} size={20} />
                          </div>
                          <div style={{ textAlign: 'left' }}>
                            <div style={activityTitle}>
                              {p.delta.displaySymbol} <span style={{ opacity: 0.5, fontWeight: 400 }}>Detected</span>
                            </div>
                            <div style={activitySub}>
                                <div style={{display: 'flex', alignItems: 'center', gap: 6}}>
                                    {chainMeta.icon && <img src={chainMeta.icon} alt="" style={{width: 14, height: 14, borderRadius: '50%'}} />}
                                    <span>{chainMeta.name}</span>
                                    <span style={{opacity: 0.3}}>•</span>
                                    <span>{new Date(p.detectedAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                </div>
                            </div>
                          </div>
                        </div>
...
function TokenChip({ symbol }: { symbol: string }) {
  return (
    <div style={chip}>
      ...
      <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.01em' }}>{symbol}</span>
    </div>
  );
}

function ChainAvatar({ chainId, size = 20 }: { chainId: number; size?: number }) {
  const url = LOGO_URLS[chainId];
  if (!url) {
    return <div style={{ ...avatarFallback, width: size, height: size }}>#</div>;
  }
  return <img src={url} alt="Chain" style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover' }} />;
}
...
function TokenAvatar({ symbol, size = 20 }: { symbol: string; size?: number }) {
  ...
  if (!url) {
    const label = s[0] || '?';
    return (
      <div style={{ ...avatarFallback, width: size, height: size, fontSize: Math.max(10, size - 8) }}>{label}</div>
    );
  }
  return <img src={url} alt={s} style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover' }} />;
}
